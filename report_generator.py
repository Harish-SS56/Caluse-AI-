...
"""
    ...
    def _generate_risk_scores_section(self, results: Dict) -> str:
        ...
        section = f"""## 📊 Risk Scoring Analysis
...
- 🔴 **Critical:** {severity_breakdown.get('critical', 0)} risks
- 🟠 **High:** {severity_breakdown.get('high', 0)} risks
- 🟡 **Medium:** {severity_breakdown.get('medium', 0)} risks
- 🟢 **Low:** {severity_breakdown.get('low', 0)} risks

"""
        if missing_penalty > 0:
            section += f"⚠️ **Missing Clause Penalty:** +{missing_penalty:.1f} points added to score due to critical missing clauses.\n\n"
...
"""
    ...
    def _generate_missing_clauses_section(self, results: Dict) -> str:
        ...
        if not missing_clauses:
            return """## ✅ Missing Clause Analysis
...
"""
        ...
        section = f"""## ❌ Missing Critical Clauses

**Contract Completeness Score:** {completion_score:.0f}%

### Summary:
- 🔴 **Critical Missing:** {critical_missing}
- 🟠 **High Priority Missing:** {high_missing}
- 🟡 **Medium Priority Missing:** {medium_missing}
...
"""
        ...
        if critical_clauses:
            section += "#### 🔴 Critical Missing Clauses\n\n"
            for clause in critical_clauses:
                section += f"**❌ {clause.get('clause_name')}**\n"
        ...
        if high_clauses:
            section += "#### 🟠 High Priority Missing Clauses\n\n"
        ...
        if medium_clauses:
            section += "#### 🟡 Medium Priority Missing Clauses\n\n"
...
- **Risks Identified:** {risk_count}
  - 🔴 Critical: {risks_by_severity['Critical']}
  - 🟠 High: {risks_by_severity['High']}
  - 🟡 Medium: {risks_by_severity['Medium']}
  - 🟢 Low: {risks_by_severity['Low']}
...
"""
    ...
    def _generate_domain_analysis(self, results: Dict) -> str:
        ...
        domains = {
            'compliance': ('📋 Compliance Analysis', results.get('compliance_analysis')),
            'finance': ('💰 Finance Analysis', results.get('finance_analysis')),
            'legal': ('⚖️ Legal Analysis', results.get('legal_analysis')),
            'operations': ('🔧 Operations Analysis', results.get('operations_analysis'))
        }
