...
class VectorStore:
    ...
    def _load_embedding_model(self):
        """Load the embedding model on demand, with a deterministic fallback."""
        global SentenceTransformer, LOCAL_EMBEDDINGS_AVAILABLE, _sentence_transformers_import_attempted
        ...
        if not _sentence_transformers_import_attempted:
            ...
            try:
                ...
                LOCAL_EMBEDDINGS_AVAILABLE = False
                print(f"[WARNING] sentence-transformers unavailable, using fallback embeddings: {str(exc)}")
                self.embedding_model = None
                return False
        ...
        if LOCAL_EMBEDDINGS_AVAILABLE and SentenceTransformer is not None:
            try:
                ...
                print(f"[WARNING] Failed to load sentence-transformers model, using fallback embeddings: {str(exc)}")
                self.embedding_model = None
                return False
    ...
    def _fallback_embedding(self, text: str) -> List[float]:
        """Generate a deterministic 384-dimension embedding without ML dependencies."""
        dimensions = 384
        values: List[float] = []
        counter = 0
    ...
    def generate_query_embedding(self, query_text: str) -> Optional[List[float]]:
        ...
        if not self.embedding_model:
            if not self._load_embedding_model():
                return self._fallback_embedding(query_text)
        
        try:
            embedding = self.embedding_model.encode([query_text], show_progress_bar=False)
            return embedding[0].tolist()
        except Exception as e:
            print(f"[WARNING] Error generating query embedding, using fallback: {str(e)}")
            return self._fallback_embedding(query_text)
